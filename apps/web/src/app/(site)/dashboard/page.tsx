...
import { DashboardView, type DashboardRegistration } from "./dashboard-view";
...
export default async function Portal() {
  ...
  if (!parent) redirect("/login?next=%2Fdashboard");
  ...
  const registrations: DashboardRegistration[] = rows.map((r) => {
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-12">
      <span className="kc-eyebrow">Dashboard</span>
      <h1 className="mt-5 font-display text-4xl font-bold leading-tight text-green-900">
        Aloha, {parent.fullName.split(" ")[0]}
      </h1>
      <p className="mt-3 text-ink-soft">
        Every class your keiki are signed up to, and when each one runs. Registered to{" "}
        {parent.email}.
      </p>
      ...
      {registrations.length === 0 ? (
        <div className="kc-card mt-10 p-8 text-center">
          ...
          <Link href="/programs" className="kc-btn kc-btn-primary mt-6">
            Browse classes
          </Link>
        </div>
      ) : (
        <DashboardView registrations={registrations} todayKey={todayKey} />
      )}
    </div>
  );
}
