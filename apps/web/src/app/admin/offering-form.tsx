...
import { saveOffering, deleteOffering, duplicateOffering } from "@/app/admin/catalogue-actions";
import { GRADE_LABELS } from "@/lib/grades";
import type { OfferingRow } from "@/app/admin/catalogue-queries";
...
export function OfferingForm({
  ...
  /**
   * A program being created alongside this class, when the picker says so.
   *
   * Held here rather than posted blind so the title can follow the name as it
   * is typed, which is what an office would otherwise do by copying and pasting.
   */
  const [newProgram, setNewProgram] = useState({ name: "", track: "", subject: "" });
  /** Once somebody edits the title themselves, stop overwriting it for them. */
  const [titleTouched, setTitleTouched] = useState(Boolean(offering?.title));
            ...
            <div className="block">
              <label className="block">
                ...
                >
                  ...
                  {/*
                    Creating the program from here, rather than sending somebody
                    to another page first.

                    "Make a program, then make a class" is the step everybody got
                    stuck on, and fairly: from the outside the two forms look
                    like the same thing asked twice. The distinction is real and
                    the data keeps it, because one program does run at several
                    campuses with a different day and price at each. It is just
                    not something an office should have to learn before adding
                    their first class.
                  */}
                  <option value="new">+ A program we do not have yet</option>
                </select>
                <span className="mt-1 block text-[var(--ops-muted)]">
                  The curriculum, for example &ldquo;Code Heroes: Virtual Reality&rdquo;. One
                  program runs as a separate class at each campus.
                </span>
              </label>

              {programId === "new" && (
                <div className="ops-newprogram mt-3">
                  <label className="block">
                    <span className="ops-label">New program name</span>
                    <input
                      name="newProgramName"
                      className="ops-field w-full"
                      placeholder="Code Heroes: Virtual Reality"
                      value={newProgram.name}
                      onChange={(e) => {
                        const name = e.target.value;
                        setNewProgram((p) => ({ ...p, name }));
                        // The class title almost always is the program name.
                        // Prefilling it saves typing the same words twice, and
                        // it stays editable for the campuses that add "(all
                        // students)" or "(non A+)" to theirs.
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <label className="block">
                      <span className="ops-label">Level</span>
                      <input
                        name="newProgramTrack"
                        className="ops-field w-full"
                        placeholder="Code Heroes"
                        value={newProgram.track}
                        onChange={(e) =>
                          setNewProgram((p) => ({ ...p, track: e.target.value }))
                        }
                      />
                    </label>
                    <label className="block">
                      ...
                      <input
                        name="newProgramSubject"
                        className="ops-field w-full"
                        placeholder="Virtual Reality"
                        value={newProgram.subject}
                        onChange={(e) =>
                          setNewProgram((p) => ({ ...p, subject: e.target.value }))
                        }
                      />
                    </label>
                  </div>
                  <p className="mt-2 text-[var(--ops-muted)]">
                    Created with this class. Its description and picture can be added on the
                    program&apos;s own page afterwards, and both are shared by every campus
                    running it.
                  </p>
                </div>
              )}
